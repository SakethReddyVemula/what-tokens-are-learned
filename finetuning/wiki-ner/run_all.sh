#!/bin/bash
#SBATCH --job-name=wikiner_all
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=20
#SBATCH --gres=gpu:4
#SBATCH --mem-per-cpu=2048
#SBATCH --time=1-00:00:00
#SBATCH --output=wiki-ner-finetuning-all.out
#SBATCH --error=wiki-ner-finetuning-all.err
    ...
    for MODEL_TYPE in "${MODEL_TYPES[@]}"; do
        ...
        for SEED in "${SEEDS[@]}"; do
            ...
            if [ "$MODEL_TYPE" == "superbpe" ]; then
                JOB_NAME="wikiner_${LANG}_${MODEL_TYPE}_${SUPERBPE_BASE_VOCAB_SIZE}_${VOCAB_SIZE}_seed${SEED}"
            else
                JOB_NAME="wikiner_${LANG}_${MODEL_TYPE}_${VOCAB_SIZE}_seed${SEED}"
            fi
