...
@dataclass
class DataArguments:
    dataset_name: str = field(default="ai4bharat/indic_glue", metadata={"help": "Dataset name"})
    dataset_config: str = field(default="wiki-ner.hi", metadata={"help": "Dataset config"})
...
def main():
    ...
    if training_args.local_rank == 0 and "wandb" in training_args.report_to:
        wandb.init(
            project=os.environ.get("WANDB_PROJECT", "p3-wiki-ner-finetuning"),
            name=f"wikiner_{data_args.dataset_config}_{model_args.language}_{'bpe-dropout' if model_args.bpe_dropout else model_args.model_type}_v{model_args.vocab_size}",
    ...
    logger.info(f"Found {num_labels} NER tags")
    ...
    def map_tokenize(samples):
        ...
        for i, words in enumerate(samples["tokens"]):
            ...
            labels = [-100]
            word_labels = samples["ner_tags"][i]
            for word, label in zip(words, word_labels):
                ...
                labels.append(label)
                # BIO Continuation: if B-tag (odd), assign I-tag (even = B+1) to subwords
                subword_label = label + 1 if label % 2 == 1 else -100
                labels.extend([subword_label] * (len(word_ids) - 1))
    ...
    with open(f"{training_args.output_dir}/ner_labels.json", 'w') as f:
