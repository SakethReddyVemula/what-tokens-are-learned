...
class ModelArguments:
    language: str = field(default="en", metadata={"help": "Language code"})
...
@dataclass
class DataArguments:
    dataset_name: str = field(default="conll2003", metadata={"help": "Dataset name"})
...
def main():
    ...
    if training_args.local_rank == 0 and "wandb" in training_args.report_to:
        wandb.init(
            project=os.environ.get("WANDB_PROJECT", "p3-ner_CoNLL-finetuning"),
            name=f"ner_conll_{model_args.language}_{'bpe-dropout' if model_args.bpe_dropout else model_args.model_type}_v{model_args.vocab_size}",
            entity="vemulasakethreddy_10"
        )
    
    # Load with a pinned revision to avoid a UnicodeDecodeError in older
    # `datasets` versions that try to read the gzip-compressed dataset script
    # as plain UTF-8 text (byte 0x8b is the second byte of a gzip magic number).
    # Falling back to the default (no revision) if the parquet ref doesn't exist.
    try:
