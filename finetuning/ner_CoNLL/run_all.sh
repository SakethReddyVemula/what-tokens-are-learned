#!/bin/bash
#SBATCH --job-name=nerconll_all
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=20
#SBATCH --gres=gpu:4
#SBATCH --mem-per-cpu=2048``
#SBATCH --time=1-00:00:00
#SBATCH --output=ner-conll-finetuning-all.out
#SBATCH --error=ner-conll-finetuning-all.err
...
export WANDB_PROJECT="p3-ner_CoNLL-finetuning"
...
# ---------------------------------------------------------------------------
# Sweep loop: tokenizer × seed  (language fixed to eng / conll2003)
# ---------------------------------------------------------------------------

TOTAL=$(( ${#MODEL_TYPES[@]} * ${#SEEDS[@]} ))
COUNT=0
...
for MODEL_TYPE in "${MODEL_TYPES[@]}"; do
    ...
    for SEED in "${SEEDS[@]}"; do
        ...
        if [ "$MODEL_TYPE" == "superbpe" ]; then
            JOB_NAME="ner_conll_${LANG}_${MODEL_TYPE}_${SUPERBPE_BASE_VOCAB_SIZE}_${VOCAB_SIZE}_seed${SEED}"
        else
            JOB_NAME="ner_conll_${LANG}_${MODEL_TYPE}_${VOCAB_SIZE}_seed${SEED}"
        fi
        ...
        echo "  Language   : $LANG ($DATASET_NAME)"
