...
class TokenizerWrapper:
    ...
    def _load_vocab(self):
        ...
        if self.hf_tokenizer is not None:
            ...
            # Compute max ID from raw values BEFORE decoding to avoid non-determinism
            # (multiple byte sequences decode to \ufffd; winner depends on Rust
            # HashMap random per-process iteration order).
...
def main():
    ...
    true_max_id_train = max(max(x) for x in dataset_train["input_ids"])
    true_max_id_eval = max(max(x) for x in dataset_eval["input_ids"]) if len(dataset_eval) > 0 else 0
    true_max_id = max(true_max_id_train, true_max_id_eval)
